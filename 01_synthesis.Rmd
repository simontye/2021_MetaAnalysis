...
get.mean <- function(x) mean(x, na.rm = TRUE)
...
# Leps 2018 (field, done)

```{r, echo = `FALSE`}
# Leps2018_Fig1.png
fig.data.1 <- fig.data %>%
  filter(filename == "Leps2018_Fig1.png") %>%
  filter(group.1 == "Forb" | group.1 == "Forb and grass") %>%
  dplyr::rowwise() %>%
  mutate(
    ### Control
    # n
    n.c    = ifelse(group.1 == "Forb", n, NA),
    # mean
    mean.c = ifelse(group.1 == "Forb", mean, NA),
    # sd
    sd.c   = ifelse(group.1 == "Forb", sd, NA),
    ### Experimental
    # n
    n.e    = ifelse(group.2 == "Forb and grass", n, 6),
    # mean
    mean.e = ifelse(group.2 == "Forb and grass", mean, NA),
    # sd
    sd.e   = ifelse(group.2 == "Forb and grass", sd, NA)) %>%
  mutate(
    # Remove old stats columns
    mean       = NULL,
    sd         = NULL,
...
fig.data.2 <- fig.data %>%
  filter(filename == "Leps2018_Fig1.png") %>%
  filter(group.1 == "Grass" | group.1 == "Forb and grass") %>%
  dplyr::rowwise() %>%
  mutate(
    ### Control
    # n
    n.c    = ifelse(group.1 == "Grass", n, NA),
    # mean
    mean.c = ifelse(group.1 == "Grass", mean, NA),
    # sd
    sd.c   = ifelse(group.1 == "Grass", sd, NA),
    ### Experimental
    # n
    n.e    = ifelse(group.2 == "Forb and grass", n, 6),
    # mean
...
### Step 6: Add local environmental data

```{r, echo = `FALSE`}
# Load climate metadata
climate.meta <- isdparser::isd_metadata

# Load study details
localities <- read.csv(file = "data/localities/ma_localities.csv", header = TRUE, sep = ",")

# Reformat localities
localities <- localities %>%
  filter(location_type == "field") %>%
  tibble::rowid_to_column(., "id") %>%
  dplyr::select(., c(id, study, experiment,
                     lat, long, elev_ft,
                     year_start, month_start, day_start,
                     year_end, month_end, day_end)) %>%
  # Convert elevation (ft; via Google Earth Pro) to meters
  dplyr::rename(elev           = elev_ft,
                year_start_exp = year_start,
                year_end_exp   = year_end) %>%
  dplyr::mutate(elev = as.numeric(elev) * 0.3048,
                lat  = as.numeric(lat),
...
for(i in 1:max(localities$id)){
  # Find nearest weather station within 100 km radius
  tmp <- isd_stations_search(lat = localities$lat[i],
                             lon = localities$long[i],
                             radius = 100)
  # Add identifier and retain closest weather station
  tmp <- tmp %>%
    mutate(id = i)
  # Bind results
  weather.stations <- rbind(tmp, weather.stations)
  # Remove objects
  rm(i, tmp)
}

# Find closest weather data that covers study period
localities <- weather.stations %>%
  separate(., col = c("begin"),
           into = c("year_start_climate", "month_start_climate", "day_start_climate"),
           sep = c(4, 6), remove = TRUE) %>%
  separate(., col = c("end"),
           into = c("year_end_climate", "month_end_climate", "day_end_climate"),
           sep = c(4, 6), remove = TRUE) %>%
  dplyr::select(., c("id", "usaf", "wban",
                     "station_name", "distance",
                     "ctry", "state", "icao",
                     "lat", "lon", "elev_m",
                     "year_start_climate", "year_end_climate")) %>%
  dplyr::rename(country = ctry,
                long    = lon,
                elev    = elev_m) %>%
  left_join(localities[c("id", "year_start_exp", "year_end_exp",
                         "study", "experiment")], ., by = c("id")) %>%
  filter(year_start_exp - 1 > year_start_climate) %>%
  filter(year_end_exp + 1 < year_end_climate) %>%
  group_by(id) %>%
...
for(i in unique(localities$id)){
  ...
  tmp <- localities %>%
    filter(id == study.id) %>%
    mutate(year_start_exp = as.numeric(year_start_exp),
           year_end_exp   = as.numeric(year_end_exp))
  }, error = function(e){})
  # Acquire climate data for each year
  for(j in (tmp$year_start_exp):(tmp$year_end_exp)){
    tryCatch({
      # Save year
      climate.year <- j
      # Isolate year
      tmp2 <- isd(usaf = tmp$usaf,
                  wban = tmp$wban,
                  year = climate.year)
      # Subset air and precipitation data
      tmp2 <- tmp2 %>%
        dplyr::select(., c("usaf_station", "wban_station",
                           "temperature", "AA1_depth")) %>%
        separate(., temperature, into = c("temperature_sign", "temperature"), sep = c(1)) %>%
        mutate(temperature = as.numeric(temperature),
               AA1_depth   = as.numeric(AA1_depth)) %>%
        mutate(temperature = ifelse(temperature_sign == "+",
                                    temperature,
                                    temperature * -1)) %>%
        dplyr::rename(usaf   = usaf_station,
                      wban   = wban_station,
                      precip = AA1_depth,
                      temp   = temperature) %>%
        filter(temp   != 9999) %>%
        ...
        dplyr::mutate(temp             = temp / 10,
                      precip           = precip / 10,
                      temperature_sign = NULL) %>%
        group_by(usaf, wban) %>%
        dplyr::summarize_all(c(get.mean, get.se)) %>%
        mutate(year_climate   = climate.year,
               year_start_exp = tmp$year_start_exp,
               year_end_exp   = tmp$year_end_exp)
        # Rename columns
        colnames(tmp2) = gsub("_fn1", ".mean", colnames(tmp2))
        colnames(tmp2) = gsub("_fn2", ".se",   colnames(tmp2))
        # Bind results
        climate <- rbind(tmp2, climate)
        }, error = function(e){})
  }
}

# Add climate data to localities
climate <- localities %>%
  left_join(., climate,
            by = c("usaf", "wban",
                   "year_start_exp", "year_end_exp")) %>%
  mutate(temp.se      = NULL,
         precip.se    = NULL,
         experiment   = NULL,
         year_climate = NULL) %>%
  group_by(id, year_start_exp, year_end_exp, study,
           usaf, wban, station_name, distance,
           country, state, icao, lat, long, elev,
           year_start_climate, year_end_climate) %>%
  dplyr::summarize_all(c(get.mean, get.se))

# Rename columns
colnames(climate) = gsub("_fn1", ".mean", colnames(climate))
colnames(climate) = gsub("_fn2", ".se",   colnames(climate))

# Add climate data to figure data
fig.data <- fig.data %>%
  separate(col = "filename",
           into = c("study"),
           remove = FALSE) %>%
  left_join(., climate, by = c("study"))
```
...
# Calculate mean effect size by interaction
es <- es %>%
  group_by(interaction) %>%
  dplyr::summarize(es     = mean(es),
                   es.var = mean(es.var)) %>%
  mutate(organism.first  = "all",
         organism.second = "all") %>%
  full_join(es, ., by = c("es", "es.var", "interaction",
                          "organism.first", "organism.second"))

# Calculate relative interaction intensity (RII)
rii <- es %>%
  group_by(interaction) %>%
  dplyr::summarize(rii = abs(mean(mean.e, na.rm = TRUE) - mean(mean.c, na.rm = TRUE)) /
                     (mean(mean.e, na.rm = TRUE) + mean(mean.c, na.rm = TRUE)))
...
ggplot(filter(es, abs(es) < 6),
  ...
  scale_color_manual(values = c("black", "mediumslateblue", "gold3", "darkseagreen3")) +
...
#                  linetype = "solid",
#                  size = 0.2) +
#  labs(y = "Organism (interacting)", x = "Effect size (Cohen's D)",
...
#       color = "Interaction") +
#  pretty +
#  facet_wrap(~organism.first, nrow = 4)
