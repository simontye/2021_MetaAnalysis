...
# Silvicultural Experiments for A-Value Controlled Single Tree Release of Future Trees



ma.search(wos.yes, 17)
```
...
# Petipas2014_Fig1
fig.data.1 <- fig.data %>%
  filter(filename == "Petipas2014_Fig1.png")
...
%>%
  ...
  mutate(
    ### Control
    # n
    n.c    = ifelse(group.1 == "AMF richness" && group.2 == "off termite mound" && group.3 == "fenced", n, NA),
    # mean
    mean.c = ifelse(group.1 == "AMF richness" && group.2 == "off termite mound" && group.3 == "fenced", mean, NA),
    # sd
    sd.c   = ifelse(group.1 == "AMF richness" && group.2 == "off termite mound" && group.3 == "fenced", sd, NA),
    ### Experimental
    # n
    n.e    = ifelse(group.1 == "AMF richness" && group.2 == "off termite mound" && group.3 == "open", n, NA),
    # mean
    mean.e = ifelse(group.1 == "AMF richness" && group.2 == "off termite mound" && group.3 == "open", mean, NA),
    # sd
    sd.e   = ifelse(group.1 == "AMF richness" && group.2 == "off termite mound" && group.3 == "open", sd, NA)) %>%
  mutate(
    # Remove old stats columns
    mean       = NULL,
    sd         = NULL,
    ...
    error_type = NULL,
    plot_type  = NULL) %>%
  filter(group.1 == "AMF richness" && group.2 == "off termite mound") %>%
  # Merge control and experimental rows
  group_by(filename, variable, article.id, group.1) %>%
  dplyr::summarize(n.c     = max(n.c, na.rm = TRUE),
                   mean.c  = max(mean.c, na.rm = TRUE),
