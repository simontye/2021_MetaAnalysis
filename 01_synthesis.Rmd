...
filenames <- data.frame(filename   = c("Bontrager2014_Fig1.png",
                        ...
                        article.id = c(226, 226, 226, 226, 226, 226,
                                       4,
...
######################################
# Schadler et a. 2010
data.figs <- metaDigitise("/Users/simontye/Research/Projects/Meta_Analysis/2021_MetaAnalysis/papers/new_figs")
...
test <- strsplit(test$group_id,',', fixed = FALSE)

# Convert list to df
test <- do.call(rbind.data.frame, test)

######## need to find efficient way to go from fig.1 to es for ma.es
######## need to find efficient way to go from fig.1 to es for ma.es
```
